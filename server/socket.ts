...
import { advanceSelectingGame, callNextNumber, claimGameWinners, DEFAULT_BOT_PURCHASE_INTERVAL_MS, getActiveGame, getTelegramProfile, persistSelectedCards, readGameState, startFinalizingGame, type GameType } from "./db";
...
export function registerGameSockets(io: Server, serviceMode: GameType = "75") {
  ...
  const advanceBots = async (gameType: GameType): Promise<number> => {
    if (botTickInProgress.has(gameType)) return DEFAULT_BOT_PURCHASE_INTERVAL_MS;
    botTickInProgress.add(gameType);
    try {
      const liveGame = await getActiveGame();
      if (liveGame.status !== "selecting") return DEFAULT_BOT_PURCHASE_INTERVAL_MS;
      const coordination = await ensureBotsForSelectingGame(String(liveGame.id));
      if (coordination.added > 0) {
        activeGames.set(gameType, String(liveGame.id));
        await broadcastState(gameType);
      }
      return coordination.intervalMs;
    } catch (error) {
      console.error(`Unable to coordinate ${gameType} bingo bots`, error);
      return DEFAULT_BOT_PURCHASE_INTERVAL_MS;
    } finally {
  ...
  let botTimerStopped = false;
  const scheduleBotTick = (intervalMs = DEFAULT_BOT_PURCHASE_INTERVAL_MS) => {
    if (botTimerStopped) return;
    botTimer = setTimeout(async () => {
      const nextIntervalMs = await advanceBots(serviceMode);
      scheduleBotTick(nextIntervalMs);
    }, intervalMs);
  };
  scheduleBotTick();
