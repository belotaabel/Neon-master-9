...
export const DEFAULT_BOT_PURCHASE_INTERVAL_MS = 10;
export const MIN_BOT_PURCHASE_INTERVAL_MS = 10;
export const MAX_BOT_PURCHASE_INTERVAL_MS = 1000;
export const DEFAULT_BOT_BATCH_MIN_SIZE = 1;
export const DEFAULT_BOT_BATCH_MAX_SIZE = 1;
export const MIN_BOT_BATCH_SIZE = 1;
export const MAX_BOT_BATCH_SIZE = 25;

function normalizeBotPurchaseInterval(value: number) {
  return Number.isInteger(value) && value >= MIN_BOT_PURCHASE_INTERVAL_MS && value <= MAX_BOT_PURCHASE_INTERVAL_MS
    ? value
    : DEFAULT_BOT_PURCHASE_INTERVAL_MS;
}

export function normalizeBotBatchSize(value: number) {
  return Number.isInteger(value) && value >= MIN_BOT_BATCH_SIZE && value <= MAX_BOT_BATCH_SIZE
    ? value
    : DEFAULT_BOT_BATCH_MIN_SIZE;
}

export async function getBotSettings(): Promise<BotSettings> {
  if (!db) throw new Error("DATABASE_URL is not configured");
  const result = await db.query<{ key: string; value: string | number }>(
    "SELECT key, value FROM app_settings WHERE key IN ('global_bots_enabled', 'global_bot_count', 'global_bot_purchase_interval_ms', 'global_bot_batch_min_size', 'global_bot_batch_size')",
  );
  const values = Object.fromEntries(result.rows.map((row) => [row.key, Number(row.value)]));
  const batchSizeMin = normalizeBotBatchSize(values.global_bot_batch_min_size ?? DEFAULT_BOT_BATCH_MIN_SIZE);
  const batchSizeMax = Math.max(batchSizeMin, normalizeBotBatchSize(values.global_bot_batch_size ?? DEFAULT_BOT_BATCH_MAX_SIZE));
  return {
    enabled: values.global_bots_enabled === 1,
    botCount: values.global_bot_count ?? 0,
    purchaseIntervalMs: normalizeBotPurchaseInterval(values.global_bot_purchase_interval_ms),
    batchSizeMin,
    batchSizeMax,
  };
}

export async function updateBotSettings(enabled: boolean, botCount: number, purchaseIntervalMs: number, batchSizeMin: number, batchSizeMax: number): Promise<BotSettings> {
  if (!db) throw new Error("DATABASE_URL is not configured");
  await db.query(
    `INSERT INTO app_settings (key, value) VALUES
      ('global_bots_enabled', $1),
      ('global_bot_count', $2),
      ('global_bot_purchase_interval_ms', $3),
      ('global_bot_batch_min_size', $4),
      ('global_bot_batch_size', $5)
     ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`,
    [enabled ? 1 : 0, botCount, purchaseIntervalMs, batchSizeMin, batchSizeMax],
  );
  return { enabled, botCount, purchaseIntervalMs, batchSizeMin, batchSizeMax };
}
