...
export const handleAdminBotSettingsUpdate: RequestHandler = async (req, res) => {
  if (!requireAdmin(req, res)) return;
  const purchaseIntervalMs = Number(req.body?.purchaseIntervalMs);
  const batchSizeMin = Number(req.body?.batchSizeMin);
  const batchSizeMax = Number(req.body?.batchSizeMax);
  if (typeof req.body?.enabled !== "boolean" || !Number.isInteger(req.body?.botCount) || req.body.botCount < 0 || req.body.botCount > 200 || !Number.isInteger(purchaseIntervalMs) || purchaseIntervalMs < 10 || purchaseIntervalMs > 1000 || !Number.isInteger(batchSizeMin) || !Number.isInteger(batchSizeMax) || batchSizeMin < 1 || batchSizeMax > 25 || batchSizeMin > batchSizeMax) {
    res.status(400).json({ error: "Bot settings require a boolean enabled value, a count from 0 to 200, an interval from 10 to 1000 milliseconds, and a batch range from 1 to 25 with minimum no greater than maximum" });
    return;
  }
  try {
    res.json(await updateBotSettings(req.body.enabled, req.body.botCount, purchaseIntervalMs, batchSizeMin, batchSizeMax));
  } catch (error) {
