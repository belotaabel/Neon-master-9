...
function BotSettingsPanel({ headers }: { headers: Record<string, string> }) {
  const [settings, setSettings] = useState<BotSettings>({ enabled: false, botCount: 0, purchaseIntervalMs: 10, batchSizeMin: 1, batchSizeMax: 1 });
  const [botCount, setBotCount] = useState("0");
  const [purchaseIntervalMs, setPurchaseIntervalMs] = useState("10");
  const [batchSizeMin, setBatchSizeMin] = useState("1");
  const [batchSizeMax, setBatchSizeMax] = useState("1");
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState("");
  ...
  useEffect(() => {
    fetch("/api/admin/bot-settings", { headers })
      ...
      .then((data) => {
        ...
        const next = { enabled: data.enabled, botCount: Number(data.botCount), purchaseIntervalMs: Number(data.purchaseIntervalMs), batchSizeMin: Number(data.batchSizeMin), batchSizeMax: Number(data.batchSizeMax) } satisfies BotSettings;
        setSettings(next);
        setBotCount(String(next.botCount));
        setPurchaseIntervalMs(String(next.purchaseIntervalMs));
        setBatchSizeMin(String(next.batchSizeMin));
        setBatchSizeMax(String(next.batchSizeMax));
      })
  ...
  const save = async (event: FormEvent) => {
    ...
    const interval = Number(purchaseIntervalMs);
    const batchMin = Number(batchSizeMin);
    const batchMax = Number(batchSizeMax);
    const response = await fetch("/api/admin/bot-settings", {
      method: "PUT",
      headers,
      body: JSON.stringify({ enabled: settings.enabled, botCount: count, purchaseIntervalMs: interval, batchSizeMin: batchMin, batchSizeMax: batchMax }),
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) {
    ...
    setPurchaseIntervalMs(String(data.purchaseIntervalMs));
    setBatchSizeMin(String(data.batchSizeMin));
    setBatchSizeMax(String(data.batchSizeMax));
    setSaved(true);
  };
